...
public:
  ...
  void addImuMeasurement(const sensor_msgs::Imu::ConstPtr &msg) 
  {
    //给他加锁，让他一直执行，直到满
    ROS_INFO("222");
    // std::lock_gurad<std::mutex> lock(buffer_mutex_)
    std::unique_lock<std::mutex> lock(buffer_mutex_);  
    //!!!注意：设置循环后不average_gravity不变了 ==> 因为第1次执行后，没有将buffer clear掉，所以第2次执行，不会加入新的imu，所以会失败
    // 设置循环，当imu_circular_buffer满的时候，释放锁，然后继续将buffer设置为true,最后才能执行getStatus
    buffer_empty_condition_.wait(lock, [this]{ return buffer_is_empty_; });  

    while (imu_circular_buffer_.size() != imu_circular_buffer_.capacity())
    {
  ...
  void getStatus(Vec3* accel_avg_in) 
  {
    //给该线程加锁，当buffer_is_full_为T时才能拿到锁
    ROS_INFO("333");
    // std::unique_lock<std::mutex> lock(buffer_mutex_);//unique_lock和cond匹配，lock_guard和cond不匹配
    std::unique_lock<std::mutex> lock(buffer_mutex_);  
    ...
    // // Reset the flag and unlock the mutex，继续等待下一次容器满后执行
    buffer_is_full_ = false;  
    // lock.unlock();  //释放锁
    imu_circular_buffer_.clear();//清除原imu_buffer，方便下一次循环
...
private:
  ...
  std::condition_variable buffer_empty_condition_;
  bool buffer_is_full_ = false;  
...
public:
  ...
  void imuCallback(const sensor_msgs::Imu::ConstPtr &msg) 
  {
    ...
    meas_handler.getStatus(&accel_avg);
    ROS_INFO("444");
